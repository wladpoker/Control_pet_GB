...
public class ConsoleMenu {
    ...
    public ConsoleMenu(PetControl Control) {
        this.petControl = Control;
    }
    ...
    public void start() {
        ...
        try (Scanner in = new Scanner(System.in, "ibm866"); Counter count = new Counter()) {
            ...
            while (flag) {
                ...
                switch (key) {
                    case "1":
                        petControl.getAllPet();
                        break;
                    case "2":
                        PetType type = menuChoice(in);
                        if (type != null) {
                            try {
                                petControl.createPet(type);
                                count.add();
    ...
    private void menuTrainPet(int petId, Scanner in) {
        ...
        while (true) {
            ...
            if (command.length() == 1 && command.equals("0"))
                return;
            if (petControl.trainPet(petId, command))
